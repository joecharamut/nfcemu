...
class Decoder(srd.Decoder):
    ...
    def decode_bits(self):
        ...
        self.wait({0: 'r'})
        lastedge = self.samplenum
        
        lastbitstart = lastedge
        lastbitend = lastedge

        while True:
            self.wait([{0: 'r'}, {0: 'f'}])
            rising = self.matched[0]
            falling = self.matched[1]
            thisedge = self.samplenum

            sampledelta = (thisedge - lastedge)
            timedelta = roundto(sampledelta / timeunit, 0.5)

            if timedelta == 1.0:
                if falling:
                    yield ('1b', lastbitend, thisedge + bitperiod)
                elif rising:
                    yield ('0b', lastbitend, thisedge + bitperiod)
                lastedge = thisedge
                lastbitend = lastedge+bitperiod
                continue
            if timedelta == 2.0:
                ...
                print(timedelta)
                lastbitend = lastbitstart+bitperiod
                yield (timedelta, lastbitstart, lastbitend)
                lastedge = thisedge
                break
    ...
    def run_decode(self):
        ...
        while True:
            # wait for SoC '1' bit
            if not edge_time:
                # measure length of pulse to calculate 1/2 time unit
                self.wait({0: 'r'})
                ...
                last_bit = 1
                edge_time = end-start
                self.put(int(last_edge), int(edge), self.out_ann, [0, ["1"]])
            ...
            if diff > (timeunit * 3):
                ...
                self.put(int(last_edge), int(edge), self.out_ann, [0, ["0"]])
